...
import 'package:car_genius_app/src/models/movie_genre_db.dart';
import 'package:flutter/material.dart';
import 'package:car_genius_app/src/models/movie_genre_model.dart';
import 'package:car_genius_app/src/views/backup_restore_page.dart';
import 'package:car_genius_app/src/views/list_movie_genre_page.dart';
...
class HomePageState extends State<HomePage> {
  ...
  void initState() {
    ...
    print(">>> config = $config");
    MainRepository.realm = Realm(config!);
    print(">>> MovieRepository.realm = ${MainRepository.realm.toString()}");

    MainRepository.realmMovies = MainRepository.realm.all<MovieModel>();

    // only call this from MovieGenreListWidget
    //MovieRepository.generateMovieGenres();

    MainRepository.realmMovieGenres =
        MainRepository.realm.all<MovieGenreModel>();

    if (MainRepository.realmMovieGenres.isEmpty) {
      print(
          ">>> Movie Genres table is empty; generating default starter values");
      MovieGenreDatabase.generateMovieGenres();
    }
          ...
          child: Consumer<MainViewModel>(builder: (context, mainVM, _) {
            ...
            return Center(
                child: ListView.builder(
                    ...
                    itemCount:
                        MainRepository.realmMovies.length, // List<Map<...>>
                    itemBuilder: (context, index) {
                      return GestureDetector(
                        child: Material(
                          ...
                          child: InkWell(
                            onTap: () {
                              var inx = MainRepository.realmMovies[index].id;
                              print(
