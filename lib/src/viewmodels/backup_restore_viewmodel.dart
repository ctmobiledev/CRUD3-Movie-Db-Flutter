...
import 'package:car_genius_app/src/models/backup_restore_db.dart';
import 'package:flutter/material.dart';
import 'package:car_genius_app/src/models/backup_restore_model.dart';
...
class BackupRestoreViewModel extends ChangeNotifier {
  ...
  void backupAllData(BuildContext buildContext) {
    ...
    BackupRestore.backupAllMovies();
    if (MainRepository.sbJSON.length > 0) {
      BackupRestoreWidgetState.txtDataJson.text =
          MainRepository.sbJSON.toString();
      DialogHelpers.showAlertDialog(
  ...
  Future<void> showConfirmRestoreDialog(
    ...
    showDialog<String>(
      ...
      if (value == 'Y') {
        MainRepository.context = buildContext; // for popup messages
        BackupRestore.restoreAllMovies(
            BackupRestoreWidgetState.txtDataJson.text);
